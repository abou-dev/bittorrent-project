...
struct mbt_file_handler {
    ...
    size_t nb_pieces;
    const char *pieces_hash; // Hash des pièces (ne pas libérer)
};
struct mbt_file_handler *mbt_file_handler_init(struct mbt_torrent *torrent)
{
    ...
    for (size_t i = 0; i < fh->nb_pieces; i++) {
        ...
        size_t piece_size = mbt_torrent_get_piece_size(torrent, i);
        p->size = piece_size;
...
void mbt_file_handler_free(struct mbt_file_handler *fh)
{
    ...
    free(fh->pieces);
    // Ne pas libérer fh->pieces_hash car il appartient au torrent
    free(fh);
}
